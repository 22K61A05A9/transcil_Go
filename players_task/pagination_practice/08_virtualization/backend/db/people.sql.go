...
const getPlayersByWindow = `-- name: GetPlayersByWindow :many
SELECT
    playerID,
    birthYear,
    birthMonth,
...
type GetPlayersByWindowParams struct {
	Limit  int32
	Offset int32
}

func (q *Queries) GetPlayersByWindow(ctx context.Context, arg GetPlayersByWindowParams) ([]Person, error) {
	rows, err := q.db.QueryContext(ctx, getPlayersByWindow, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
