...
func SetupMerchantRoutes(router *gin.Engine, h *handlers.Handler) {
	...
	router.POST("/merchants/register", h.RegisterMerchant)
	// Public catalog for purchase UI (must be before /merchants/:id).
	router.GET("/merchants/available", h.GetAvailableMerchants)
	...
	// Merchant management (admin-protected)
