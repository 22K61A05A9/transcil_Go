...
const getAvailableMerchants = `-- name: GetAvailableMerchants :many
SELECT id, merchant_name
FROM merchants
ORDER BY merchant_name ASC, id ASC
`

type GetAvailableMerchantsRow struct {
	ID           int32  `json:"id"`
	MerchantName string `json:"merchant_name"`
}

func (q *Queries) GetAvailableMerchants(ctx context.Context) ([]GetAvailableMerchantsRow, error) {
	rows, err := q.db.QueryContext(ctx, getAvailableMerchants)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetAvailableMerchantsRow
	for rows.Next() {
		var i GetAvailableMerchantsRow
		if err := rows.Scan(&i.ID, &i.MerchantName); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
